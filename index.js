...
class scrapy {
  ...
  async get_data(_url) {
    ...
    var data = await this.frame.evaluate(() => {
      ...
      var data = [...document.querySelectorAll("a.msk")].map(e => [
        e.title,
        e.href
      ]);
      var next_url = document.querySelector(".u-page>a:last-child").href;
      var current_page_number = document.querySelector(".u-page>a:nth-child(2)")
        .text;
  ...
  async main() {
    ...
    var [data, next_url, number] = await this.get_data(
      "https://music.163.com/#/discover/playlist/?cat=%E6%B5%81%E8%A1%8C"
